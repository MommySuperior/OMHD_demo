...
twitter_text_files = Path(__file__).parent.parent / "data" / "post_txt" / "twitter_txt"

twitter_posts = load_posts(twitter_text_files)

platform_stopwords = ["don", "ve"]
...
topics, probabilities = topic_model.fit_transform(twitter_posts)
hierarchical_topics = topic_model.hierarchical_topics(twitter_posts)

# storing topic-word representations
topic_info = topic_model.get_topic_info()
topic_info.to_csv(Path(__file__).parent.parent / "output" / "topics" / "twitter_topics.csv", index=False)

# visualization
topic_model.visualize_documents(twitter_posts).write_html(Path(__file__).parent.parent / "output" / "figures" / "twitter_documents.html")
topic_model.visualize_documents(twitter_posts).write_image(Path(__file__).parent.parent / "output" / "figures" / "twitter_documents.png")

# visualization hierarchy
topic_model.visualize_hierarchy(hierarchical_topics=hierarchical_topics).write_html(Path(__file__).parent.parent / "output" / "figures" / "twitter_hierarchy.html")
