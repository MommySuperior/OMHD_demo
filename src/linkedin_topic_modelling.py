...
linkedin_text_files = Path(__file__).parent.parent / "data" / "post_txt" / "linkedin_txt"

linkedin_posts = load_posts(linkedin_text_files)

platform_stopwords = ["ll"]
...
topics, probabilities = topic_model.fit_transform(linkedin_posts)
hierarchical_topics = topic_model.hierarchical_topics(linkedin_posts)

# storing topic-word representations
topic_info = topic_model.get_topic_info()
topic_info.to_csv(Path(__file__).parent.parent / "output" / "topics" / "linkedin_topics.csv", index=False)

# visualization documents
topic_model.visualize_documents(linkedin_posts).write_html(Path(__file__).parent.parent / "output" / "figures" / "linkedin_documents.html")
topic_model.visualize_documents(linkedin_posts).write_image(Path(__file__).parent.parent / "output" / "figures" / "linkedin_documents.png")

# visualization hierarchy
topic_model.visualize_hierarchy(hierarchical_topics=hierarchical_topics).write_html(Path(__file__).parent.parent / "output" / "figures" / "linkedin_hierarchy.html")
